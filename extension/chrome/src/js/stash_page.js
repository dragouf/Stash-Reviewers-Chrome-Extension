(function() {
	...
	define('stash-plugin/url', function(){
				...
				window.communication.runtime.sendMessage(window.chromeExtId, {
					...
					if(newVersion !== storedVersion && newVersion !== currentVersion) {
						...
						if (data.response === 'cant.reach.github') {
							body = "Please check you added your stash server domain to extension manifest.json";
						}
			...
			if(user) {
				require(['stash-plugin/header-notification'], function(notification) {
					...
					if(!project) {
						...
						require(['stash-plugin/branch-details-page', 'util/events'], function(branchUtils, events){
